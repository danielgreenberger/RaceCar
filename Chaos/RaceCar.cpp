...
RaceCar::RaceCar()
{
    ...
    /* ROS ONLY -- Init ros infrastructure */
	ROS_CODE_SECTION
	(
...
RaceCar &RaceCar::connect(const string& ip, const unsigned short& port,const string& server_ip)
{
    ...
    if ( _camera.isConnect() ){
         ...
         std::cout << "Camera connected" <<std::endl; // dgreenbe todo is this typo
    } else {
        std::cout << "Camera NOT CONNECTED" <<std::endl; // dgreenbe todo is this typo
    }
    ...
    /* Start TCP server */
    _tcp_server->bind(server_ip, SERVER_PORT, MAX_NUM_USERS); //to allow others to connect
...
RaceCar &RaceCar::run()
{
    ...
    /* Start reading camera output and send to remote host */
    ...
    /* Get car commands from remote host and execute */
    ...
    /* Start getting bitcraze (optical flow) data */
...
RaceCar &RaceCar::getCameraOutputAndSendToRemote()
{
    ...
    while (_is_running && _tcp_client->isConnected())
    {
        ...
        //////////////////////////////////////////////
        ///    ROS only  --- publish data to ROS   ///
        //////////////////////////////////////////////

        /* 
            dgreenbe todo find out:
            1. Is this the best place to publish to the topic, or should we create a separate thread?
            2. Which type of image to publish - match RealSense types with ROS types
            3. Maybe choose 1 default type and the rest will be published via a service call?
            4. How does the RealSense API deal with concurrency? we might want to use a different thread for ROS publications
        */
        ...
        (
            ...
            m_publisher_point_cloud->Publish(ros_pc);
            
            
			// Create msg
	//		std_msgs::String msg;
	//		std::stringstream ss;
	//		ss << "hello world " << m_msg_counter++;
	//		msg.data = ss.str();
	//		
	//		// Publish
	//		m_publisher_1_p->Publish(msg);
